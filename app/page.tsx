...
export default function SpecuroDashboard() {
  ...
  // 当 items 或筛选条件改变时，更新显示的列表
  useEffect(() => {
    let result = items;
    if (activeCategory !== 'ALL') {
      result = result.filter(item => item.type?.toUpperCase().includes(activeCategory));
    }
    if (activeStyle !== 'ALL') {
      result = result.filter(item => item.style?.toUpperCase() === activeStyle);
    }
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-white text-black p-10">
        ...
        <div className="w-full max-w-xs space-y-4">
          <input type="email" placeholder="WORK EMAIL" className="w-full border-b border-black py-3 text-sm font-bold outline-none" value={email} onChange={e => setEmail(e.target.value)} />
          ...
          <button onClick={handleAuth} className="w-full bg-black text-white py-4 text-[10px] font-black tracking-widest uppercase hover:bg-gray-800 transition-all">
            {isSignUp ? "Create Private Account" : "Enter the Archive"}
          </button>
          <button onClick={() => setIsSignUp(!isSignUp)} className="w-full text-[9px] font-bold text-gray-400 uppercase tracking-tighter hover:text-black transition-colors">
            {isSignUp ? "Already have access? Sign In" : "Need access? Request with Invitation Code"}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-black px-10 py-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-10">
          <a href="/specuro-clipper.zip" download className="flex items-center gap-2 group">
            <div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></div>
            <span className="text-[10px] font-black border-b border-black group-hover:text-red-600 transition-all uppercase">Get Clipper v1.0</span>
          </a>
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-bold text-gray-300 uppercase">{user.email}</span>
            <button onClick={() => supabase.auth.signOut()} className="text-[9px] font-black border border-black px-2 py-1 hover:bg-black hover:text-white transition-all uppercase">Logout</button>
          </div>
        </div>
      </nav>

      {/* 悬浮筛选器 (Bauhaus Floating Controller) */}
        ...
        <div className="flex gap-6 border-r border-gray-800 pr-10">
          {['ALL', 'SEATING', 'LIGHTS', 'TABLES'].map(cat => (
            <button 
              ...
              className={`text-[9px] font-black tracking-widest transition-colors ${activeCategory === cat ? 'text-red-500' : 'hover:text-red-500'}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="flex gap-6">
          {['ALL', 'MODERN', 'MINIMAL', 'INDUSTRIAL'].map(style => (
            <button 
              ...
              className={`text-[9px] font-black tracking-widest transition-colors ${activeStyle === style ? 'text-white' : 'text-gray-500 hover:text-white'}`}
            >
              {style}
            </button>
          ))}
      ...
      <main className="pt-40 pb-40 px-10 max-w-[1600px] mx-auto">
        <header className="mb-20">
          <h2 className="text-7xl font-black tracking-tighter uppercase mb-2">My Archive.</h2>
        ...
        {loading ? (
          <div className="text-[10px] font-bold tracking-widest animate-pulse">SYNCING_DATABASE...</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-24">
            {filteredItems.map((item) => (
              <div key={item.id} className="group">
                {/* 图片区域 - 恢复彩色，取消 grayscale */}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 border-t border-gray-100 pt-4 gap-4">
                    ...
                    <div>
                      <label className="block mb-1">Lead Time</label>
